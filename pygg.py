...
DEFAULT_OUT = "out"
SUFFIX = "" #"-static"
REQUIRED_BINS = ["gg-create-thunk" + SUFFIX, "gg-hash" + SUFFIX]
...
class GG:
    ...
    def _hash_file(self, path: str) -> Hash:
        return (
            sub.check_output([self.bin("gg-hash" + SUFFIX).path(), path]).decode().strip()
        )
    ...
    def _save_thunk(self, t: Thunk, dest_path: Optional[str]) -> Hash:
        ...
        cmd_args = list(
            it.chain(
                [self.bin("gg-create-thunk" + SUFFIX).path()],
